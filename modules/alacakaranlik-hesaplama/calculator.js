function hcAKSelectCity() {
    const cityVal = document.getElementById('hc-ak-city').value;
    if (cityVal) {
        const coords = cityVal.split(',');
        document.getElementById('hc-ak-lat').value = coords[0];
        document.getElementById('hc-ak-lng').value = coords[1];
    }
}

function hcAKGetLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            document.getElementById('hc-ak-lat').value = position.coords.latitude.toFixed(4);
            document.getElementById('hc-ak-lng').value = position.coords.longitude.toFixed(4);
        });
    }
}
...
function hcAlacakaranlikHesapla() {
    const dateStr = document.getElementById('hc-ak-date').value;
    const lat = parseFloat(document.getElementById('hc-ak-lat').value);
    const lng = parseFloat(document.getElementById('hc-ak-lng').value);
    if (!dateStr || isNaN(lat) || isNaN(lng)) { alert('Lütfen bilgileri girin.'); return; }
    const date = new Date(dateStr);
    const result = computeSunTime(date, lat, lng, 96, false);
    document.getElementById('hc-ak-val').innerText = result;
    document.getElementById('hc-ak-info').innerText = "Sivil alacakaranlık bitişi (Güneş ufkun 6° altında).";
