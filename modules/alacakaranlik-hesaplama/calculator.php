...
function hc_render_alacakaranlik_hesaplama( $atts ) {
    wp_enqueue_script(
        'hc-alacakaranlik',
        HC_PLUGIN_URL . 'modules/alacakaranlik-hesaplama/calculator.js',
        [], HC_VERSION, true
    );
    ?>
    <div class="hc-calculator" id="hc-alacakaranlik-hesaplama">
        <h3>Alacakaranlık Hesaplama</h3>
        <div class="hc-form-group">
            <label>Tarih Seçin</label>
            <input type="date" id="hc-ak-date" value="<?php echo date('Y-m-d'); ?>">
        </div>
        <div class="hc-form-group">
            <label>İl Seçin (Opsiyonel)</label>
            <select id="hc-ak-city" onchange="hcAKSelectCity()">
        ...
        <div class="hc-form-group">
            ...
            <div style="display: flex; gap: 10px;">
                <input type="number" id="hc-ak-lat" placeholder="Enlem" step="0.0001">
                <input type="number" id="hc-ak-lng" placeholder="Boylam" step="0.0001">
            </div>
            <button class="hc-share-btn" onclick="hcAKGetLocation()" style="margin-top: 10px; width: 100%;">
                📍 Konumumu Bul
            </button>
        </div>
        <button class="hc-btn" onclick="hcAlacakaranlikHesapla()">Hesapla</button>
        <div class="hc-result" id="hc-ak-result">
            <div class="hc-result-title">Alacakaranlık Vakti:</div>
            <div class="hc-result-value" id="hc-ak-val">-</div>
            <div style="margin-top: 10px; font-size: 13px; color: var(--hc-front-muted);" id="hc-ak-info"></div>
        </div>
    </div>
    <?php
}
