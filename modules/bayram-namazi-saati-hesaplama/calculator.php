...
function hc_render_bayram_namazi_saati_hesaplama( $atts ) {
    wp_enqueue_script(
        'hc-bayram-hesaplama',
        HC_PLUGIN_URL . 'modules/bayram-namazi-saati-hesaplama/calculator.js',
        [], HC_VERSION, true
    );
    ?>
    <div class="hc-calculator" id="hc-bayram-wrapper">
        <h3>Bayram Namazı Saati Hesaplama</h3>
        <div class="hc-form-group">
            <label>Bayram Seçin (2026)</label>
            <select id="hc-bn-type">
                <option value="2026-03-20">Ramazan Bayramı (1. Gün)</option>
                <option value="2026-05-27">Kurban Bayramı (1. Gün)</option>
            </select>
        </div>
        <div class="hc-form-group">
            <label>İl Seçin (Opsiyonel)</label>
            <select id="hc-bn-city" onchange="hcBNSelectCity()">
        ...
        <div class="hc-form-group">
            ...
            <div style="display: flex; gap: 10px;">
                <input type="number" id="hc-bn-lat" placeholder="Enlem" step="0.0001">
                <input type="number" id="hc-bn-lng" placeholder="Boylam" step="0.0001">
            </div>
        </div>
        <button class="hc-btn" onclick="hcBayramHesapla()">Bayram Vaktini Hesapla</button>
        <div class="hc-result" id="hc-bn-result">
            <div class="hc-result-title">Bayram Namazı Vakti:</div>
            <div class="hc-result-value" id="hc-bn-val">-</div>
            <div style="margin-top: 10px; font-size: 13px; color: var(--hc-front-muted);">
                * Bayram namazı vakti, güneş doğuşundan yaklaşık 45-50 dakika sonra kerahat vaktinin çıkışıyla başlar.
            </div>
        </div>
    </div>
    <?php
