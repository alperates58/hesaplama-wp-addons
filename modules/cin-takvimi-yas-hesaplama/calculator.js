function hcCinYasiHesapla() {
    const bInput = document.getElementById('hc-ca-date').value;
    if (!bInput) {
        alert('Lütfen doğum tarihinizi seçin.');
        return;
    }

    const birthDate = new Date(bInput);
    ...
    // Traditional Chinese age (Sui) is Current Lunar Year - Birth Lunar Year + 1
    const suiAge = (currentYear - birthYear) + 1;

    let desc = `<p>Geleneksel Çin yaş sistemine (Sui) göre, bir bebek doğduğunda 1 yaşında kabul edilir ve her Çin Yeni Yılı'nda bir yaş daha alır.</p>`;
    desc += `<p>Batı takvimine göre yaşınız ${currentYear - birthYear} iken, Çin takvimine göre şu an <strong>${suiAge}</strong> yaşındasınız.</p>`;

    document.getElementById('hc-ca-value').innerText = suiAge;
    document.getElementById('hc-ca-desc').innerHTML = desc;
    document.getElementById('hc-cin-takvimi-yas-result').classList.add('visible');
}
