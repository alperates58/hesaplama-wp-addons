function hcElementDagilimiHesapla() {
    const dStr = document.getElementById('hc-elem-date').value;
    const tStr = document.getElementById('hc-elem-time').value;
    const cVal = document.getElementById('hc-elem-city').value;
    ...
    const heroHtml = `
        <div class="hc-elem-hero-card">
            <div class="hc-elem-hero-badge">Baskın Element</div>
            <div class="hc-elem-hero-title">${dominantElem.name} Elementi (%${dominantElem.pct})</div>
            <p class="hc-elem-hero-sub">Haritanızda en yoğun çalışan enerji ${dominantElem.name} elementidir. Bu durum temel davranış kalıplarınızı ve motivasyonunuzu şekillendirir.</p>
        </div>
    `;
    ...
    elemData.forEach(item => {
        barsHtml += `
            <div class="hc-elem-bar-row">
                <div class="hc-elem-bar-header">
                    <span class="hc-elem-name">${item.name} Elementi</span>
                    <span class="hc-elem-pct">%${item.pct} (${item.score} Puan)</span>
                </div>
                <div class="hc-elem-bar-track">
                    <div class="hc-elem-bar-fill" style="width: ${item.pct}%; background: ${elemColors[item.name]};"></div>
                </div>
            </div>
        `;
    });
    ...
    bodies.forEach(b => {
        ...
        tableHtml += `
            <tr>
                ...
                <td><span class="hc-elem-tag hc-el-${elem.toLowerCase()}">${elem}</span></td>
                <td>${b.weight} Puan</td>
            </tr>
        `;
    });
    ...
    const interpretations = {
        "Ateş": "Ateş elementinin yüksek olması size bitmek bilmeyen bir yaşam enerjisi, cesaret, liderlik ve vizyon kazandırır.",
        "Toprak": "Toprak elementinin yüksek olması ayaklarınızın yere sağlam basmasını, finansal ve pratik konularda üstün başarıyı getirir.",
        "Hava": "Hava elementinin yüksek olması güçlü iletişim, entelektüel merak, sosyallik ve tarafsız analiz yeteneği sağlar.",
        "Su": "Su elementinin yüksek olması derin empati, sezgisel bilgelik, sanatsal ilham ve güçlü duygusal bağlar kurma gücü verir."
    };

    const descHtml = `
        <p>${interpretations[dominantElem.name]}</p>
        <p><strong>Element Dengesi:</strong> Dört elementin dengeli dağılımı, kişinin hayatın hem maddi (Toprak), hem duygusal (Su), hem zihinsel (Hava), hem de ruhsal/eylemsel (Ateş) boyutlarında dengeli olmasını sağlar.</p>
    `;

    document.getElementById('hc-elem-hero').innerHTML = heroHtml;
    document.getElementById('hc-element-bars').innerHTML = barsHtml;
    document.getElementById('hc-elem-planets-table').innerHTML = tableHtml;
    document.getElementById('hc-element-desc').innerHTML = descHtml;

    document.getElementById('hc-element-dagilimi-result').classList.add('visible');
    document.getElementById('hc-element-dagilimi-result').scrollIntoView({ behavior: 'smooth', block: 'start' });
}
