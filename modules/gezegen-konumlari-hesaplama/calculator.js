function hcGezegenKonumlariHesapla() {
    const tarihStr = document.getElementById('hc-planets-date').value;
    ...
    function getHeliocentric(planet, d) {
        if (!planet.a) return { x: 0, y: 0, z: 0 }; // Sun case handled differently
    ...
    const pE = getHeliocentric(planets.earth, d);
    const results = [];

    // Sun (Geocentric = opposite of Earth's heliocentric position)
    const sunLon = norm(Math.atan2(-pE.y, -pE.x) / rad);
    results.push({ name: "Güneş", lon: sunLon });

    // Moon (Simplified LLR)
    let Ms = norm(357.5291 + 0.98560028 * d);
    let Mm = norm(134.9634 + 13.06499295 * d);
    let F = norm(93.2721 + 13.22935044 * d);
    let D = norm(297.8502 + 12.19074912 * d);
    let moonLon = sunLon + 180 + 6.289 * Math.sin(Mm * rad) + 1.274 * Math.sin((2*D - Mm) * rad) + 0.658 * Math.sin(2*D * rad) + 0.214 * Math.sin(2*Mm * rad);
    results.push({ name: "Ay", lon: norm(moonLon) });

    for (let p in planets) {
        if (p === 'earth') continue;
        const pP = getHeliocentric(planets[p], d);
        const xG = pP.x - pE.x;
        const yG = pP.y - pE.y;
        const lonG = norm(Math.atan2(yG, xG) / rad);
        results.push({ name: p.charAt(0).toUpperCase() + p.slice(1).replace('mercury','Merkür').replace('venus','Venüs').replace('jupiter','Jüpiter').replace('saturn','Satürn'), lon: lonG });
    }

    const burclar = ["Koç", "Boğa", "İkizler", "Yengeç", "Aslan", "Başak", "Terazi", "Akrep", "Yay", "Oğlak", "Kova", "Balık"];
    
    let html = "<table><thead><tr><th>Gezegen</th><th>Burç</th><th>Derece</th></tr></thead><tbody>";
    results.forEach(p => {
        const burcIdx = Math.floor(p.lon / 30);
        const derece = (p.lon % 30).toFixed(1);
        html += `<tr><td>${p.name}</td><td>${burclar[burcIdx]}</td><td>${derece}°</td></tr>`;
    });
    html += "</tbody></table>";

    document.getElementById('hc-planets-table').innerHTML = html;
    document.getElementById('hc-planets-result').classList.add('visible');
}
