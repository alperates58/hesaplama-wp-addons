...
function hc_render_gun_batimi_hesaplama( $atts ) {
    wp_enqueue_script(
        'hc-gun-batimi',
        HC_PLUGIN_URL . 'modules/gun-batimi-hesaplama/calculator.js',
        [], HC_VERSION, true
    );
    ?>
    <div class="hc-calculator" id="hc-gun-batimi-hesaplama">
        <h3>Gün Batımı Hesaplama</h3>
        <div class="hc-form-group">
            <label>Tarih Seçin</label>
            <input type="date" id="hc-gb-date" value="<?php echo date('Y-m-d'); ?>">
        </div>
        <div class="hc-form-group">
            <label>İl Seçin (Opsiyonel)</label>
            <select id="hc-gb-city" onchange="hcGBSelectCity()">
        ...
        <div class="hc-form-group">
            ...
            <div style="display: flex; gap: 10px;">
                <input type="number" id="hc-gb-lat" placeholder="Enlem" step="0.0001">
                <input type="number" id="hc-gb-lng" placeholder="Boylam" step="0.0001">
            </div>
            <button class="hc-share-btn" onclick="hcGBGetLocation()" style="margin-top: 10px; width: 100%;">
                📍 Konumumu Bul
            </button>
        </div>
        <button class="hc-btn" onclick="hcGunBatimiHesapla()">Hesapla</button>
        <div class="hc-result" id="hc-gb-result">
            <div class="hc-result-title">Gün Batımı Saati:</div>
            <div class="hc-result-value" id="hc-gb-val">-</div>
            <div style="margin-top: 10px; font-size: 13px; color: var(--hc-front-muted);" id="hc-gb-info"></div>
        </div>
    </div>
    <?php
}
