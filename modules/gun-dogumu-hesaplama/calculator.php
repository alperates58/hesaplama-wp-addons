...
function hc_render_gun_dogumu_hesaplama( $atts ) {
    wp_enqueue_script(
        'hc-gun-dogumu-hesaplama',
        HC_PLUGIN_URL . 'modules/gun-dogumu-hesaplama/calculator.js',
        [], HC_VERSION, true
    );
    ?>
    <div class="hc-calculator" id="hc-gun-dogumu-hesaplama">
        <h3>Gün Doğumu Hesaplama</h3>
        <div class="hc-form-group">
            <label>Tarih Seçin</label>
            <input type="date" id="hc-gd-date" value="<?php echo date('Y-m-d'); ?>">
        </div>
        <div class="hc-form-group">
            <label>İl Seçin (Opsiyonel)</label>
            <select id="hc-gd-city" onchange="hcGDSelectCity()">
        ...
        <div class="hc-form-group">
            ...
            <div style="display: flex; gap: 10px;">
                <input type="number" id="hc-gd-lat" placeholder="Enlem" step="0.0001">
                <input type="number" id="hc-gd-lng" placeholder="Boylam" step="0.0001">
            </div>
            <button class="hc-share-btn" onclick="hcGDGetLocation()" style="margin-top: 10px; width: 100%;">
                📍 Konumumu Bul
            </button>
        </div>
        <button class="hc-btn" onclick="hcGunDogumuHesapla()">Hesapla</button>
        <div class="hc-result" id="hc-gd-result">
            <div class="hc-result-title">Gün Doğumu Saati:</div>
            <div class="hc-result-value" id="hc-gd-val">-</div>
            <div style="margin-top: 10px; font-size: 13px; color: var(--hc-front-muted);" id="hc-gd-info"></div>
        </div>
    </div>
    <?php
}
