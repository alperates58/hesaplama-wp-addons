function hcLisAddRow() {
    const container = document.getElementById('hc-lis-rows');
    const row = document.createElement('div');
    row.className = 'hc-form-group hc-lis-row';
...
function hcLisHesapla() {
    const rows = document.querySelectorAll('.hc-lis-row');
    let totalPoints = 0;
    let totalCredits = 0;

    rows.forEach(row => {
        const grade = parseFloat(row.querySelector('.hc-lis-grade').value);
        const credit = parseFloat(row.querySelector('.hc-lis-credit').value);
    ...
    document.getElementById('hc-lis-val').innerText = gpa;
    document.getElementById('hc-lis-result').classList.add('visible');
}
