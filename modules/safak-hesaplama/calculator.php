...
function hc_render_safak_hesaplama( $atts ) {
    wp_enqueue_script(
        'hc-safak',
        HC_PLUGIN_URL . 'modules/safak-hesaplama/calculator.js',
        [], HC_VERSION, true
    );
    ?>
    <div class="hc-calculator" id="hc-safak-hesaplama">
        <h3>Şafak Hesaplama</h3>
        <div class="hc-form-group">
            <label>Tarih Seçin</label>
            <input type="date" id="hc-sf-date" value="<?php echo date('Y-m-d'); ?>">
        </div>
        <div class="hc-form-group">
            <label>İl Seçin (Opsiyonel)</label>
            <select id="hc-sf-city" onchange="hcSFSelectCity()">
        ...
        <div class="hc-form-group">
            ...
            <div style="display: flex; gap: 10px;">
                <input type="number" id="hc-sf-lat" placeholder="Enlem" step="0.0001">
                <input type="number" id="hc-sf-lng" placeholder="Boylam" step="0.0001">
            </div>
            <button class="hc-share-btn" onclick="hcSFGetLocation()" style="margin-top: 10px; width: 100%;">
                📍 Konumumu Bul
            </button>
        </div>
        <button class="hc-btn" onclick="hcSafakHesapla()">Hesapla</button>
        <div class="hc-result" id="hc-sf-result">
            <div class="hc-result-title">Şafak Vakti:</div>
            <div class="hc-result-value" id="hc-sf-val">-</div>
            <div style="margin-top: 10px; font-size: 13px; color: var(--hc-front-muted);" id="hc-sf-info"></div>
        </div>
    </div>
    <?php
}
