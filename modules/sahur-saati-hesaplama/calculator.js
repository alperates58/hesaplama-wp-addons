function hcSHSelectCity() {
    const cityVal = document.getElementById('hc-sh-city').value;
    if (cityVal) {
        const coords = cityVal.split(',');
        document.getElementById('hc-sh-lat').value = coords[0];
        document.getElementById('hc-sh-lng').value = coords[1];
    }
}
...
function hcSahurHesapla() {
    const dateStr = document.getElementById('hc-sh-date').value;
    const lat = parseFloat(document.getElementById('hc-sh-lat').value);
    const lng = parseFloat(document.getElementById('hc-sh-lng').value);
    if (!dateStr || isNaN(lat) || isNaN(lng)) { alert('Lütfen bilgileri girin.'); return; }
    const date = new Date(dateStr);
    const imsak = computeTime(date, lat, lng, 108, true);
    document.getElementById('hc-sh-val').innerText = imsak || "Hesaplanamadı";
